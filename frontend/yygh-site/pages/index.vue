...
const DEFAULT_XIEHE_HOSPITAL = {
  id: 'public-xiehe-hospital',
  hoscode: '1000_0',
  hosname: '北京协和医院',
  logoData: '',
  param: {
...
function ensureXiehe(list) {
  const safeList = Array.isArray(list) ? list : []
  const exists = safeList.some(item => item && (item.hoscode === DEFAULT_XIEHE_HOSPITAL.hoscode || item.hosname === DEFAULT_XIEHE_HOSPITAL.hosname))
  return exists ? safeList : [DEFAULT_XIEHE_HOSPITAL].concat(safeList)
}
...
export default {
  ...
  asyncData({ params, error }) {
    return hospitalApi.getPageList(1, 10, null).then(response => {
      ...
      return hospitalApi.getByHosname('北京协和医院').then(searchResponse => {
        const xiehe = (searchResponse.data || [])[0]
        return {
          list: xiehe && !list.some(item => item.hoscode === xiehe.hoscode) ? [xiehe].concat(list) : ensureXiehe(list),
          pages : response.data.totalPages
        }
      }).catch(() => ({
        list: ensureXiehe(list),
        pages : response.data.totalPages
      }))
    }).catch(() => {
      return {
  ...
  mounted() {
    this.updateHeaderSearch(false)
    this.scrollWrap = document.querySelector('.page-component__scroll .el-scrollbar__wrap')

    // 添加滚动事件，检测滚动到页面底部
    if (this.scrollWrap) {
      this.scrollWrap.addEventListener('scroll', this.load)
    } else {
  ...
  destroyed(){
    ...
    if (this.scrollWrap) {
      this.scrollWrap.removeEventListener('scroll', this.load)
    }
    window.removeEventListener('scroll', this.load, true)
    this.updateHeaderSearch(true)
  },
  ...
  methods: {
    ...
    updateHeaderSearch(visible) {
      if (process.client) {
        window.dispatchEvent(new CustomEvent('yygh-header-search-visible', {
          detail: { visible }
        }))
      }
    },
    ...
    load(event) {
      const target = event && event.target ? event.target : this.scrollWrap
      if (!target) return

      // 滚动条高度为430 页面搜索消失，头部搜索显示
      this.updateHeaderSearch(target.scrollTop > 430)
    ...
    getList() {
      hospitalApi.getPageList(this.page, this.limit, this.searchObj).then(response => {
        for(let i in response.data.content){
          if (!this.list.some(item => item.hoscode === response.data.content[i].hoscode)) {
            this.list.push(response.data.content[i]);
          }
        }
        this.pages = response.data.totalPages
        this.ensureXieheVisible()
      })
    },

    ensureXieheVisible() {
      const noFilter = !this.searchObj.hostype && !this.searchObj.districtCode
      if (!noFilter || this.list.some(item => item.hosname === '北京协和医院' || item.hoscode === '1000_0')) {
        return
      }
      hospitalApi.getByHosname('北京协和医院').then(response => {
        const item = (response.data || [])[0]
        if (item && !this.list.some(row => row.hoscode === item.hoscode)) {
