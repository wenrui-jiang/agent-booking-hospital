...
public class AgentController {
    ...
    @Value("${agent.internal-tool.secret:local-dev-agent-secret}")
    private String internalToolSecret;
    ...
    @PostMapping("internal/tool/execute")
    public Result<Object> executeTool(@RequestBody AgentToolExecuteRequest request,
                                      @RequestHeader(value = "X-Agent-Internal-Secret", required = false) String secret,
                                      @RequestHeader(value = "token", required = false) String token) {
        if (internalToolSecret != null && internalToolSecret.length() > 0
                && (secret == null || !internalToolSecret.equals(secret))) {
            return Result.fail("agent internal tool secret mismatch");
        }
        if (request.getToken() == null || request.getToken().length() == 0) {
            request.setToken(token);
        }
        return Result.ok(agentWorkflowService.executeAgentTool(request));
    }
}
